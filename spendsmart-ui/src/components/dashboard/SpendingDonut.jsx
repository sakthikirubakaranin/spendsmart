...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="glass rounded-xl px-4 py-3 shadow-xl" style={{ border: '1px solid var(--border-medium)' }}>
      <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{d.icon} {d.name}</p>
      <p className="text-lg font-bold mt-0.5 text-violet-500">{formatINR(d.amount)}</p>
      <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{d.pct}% of total</p>
    </div>
  )
}
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-2">
            {chartData.map(cat => (
              <div key={cat.category_id} className="flex items-center gap-2">
                ...
                <span className="text-xs ml-auto" style={{ color: 'var(--text-muted)' }}>{cat.pct}%</span>
              </div>
            ))}
          </div>
        </>
